...
namespace PandoraTest1.States
{
    public class MainMenu : GameState
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            town.Draw(gameTime);
            if (drawCrosshair) {
                Vector2 hor1 = new Vector2(0, InputManager.Mouse.MouseCoords.Y);
                Vector2 hor2 = new Vector2(Main.graphics.GraphicsDevice.Viewport.Width, InputManager.Mouse.MouseCoords.Y);
                Vector2 ver1 = new Vector2(InputManager.Mouse.MouseCoords.X, 0);
                Vector2 ver2 = new Vector2(InputManager.Mouse.MouseCoords.X, Main.graphics.GraphicsDevice.Viewport.Height);

                Main.spriteBatch.DrawLine(hor1, hor2, Color.Red,13);
                Main.spriteBatch.DrawLine(ver1, ver2, Color.Blue,13);
                Main.spriteBatch.DrawBox(new Rectangle((int)InputManager.Mouse.MouseCoords.X - 8, (int)InputManager.Mouse.MouseCoords.Y - 8, 16, 16), Color.White);
                Main.spriteBatch.DrawRect(new Rectangle((int)InputManager.Mouse.MouseCoords.X - 1, (int)InputManager.Mouse.MouseCoords.Y - 1, 3, 3), Color.Green);
            }
        }
    }
}
